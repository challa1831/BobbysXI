...
(function(){
  ...
  T._cdmxCell = (c) => `<div class="cdmx-cell cdmx-${c.kind || 'pl'}">
    <span class="cdmx-tag">${c.label || ''}</span>
    <span class="cdmx-v">${c.text || ''}${c.loc ? `<span class="cdmx-loc">${c.loc}</span>` : ''}</span>
  </div>`;
  T._cdmxStack = (cells) => {
    if (!cells || !cells.length) return `<div class="cdmx-empty">—</div>`;
    return cells.map(T._cdmxCell).join('');
  };
  T.cdmxPlanRow = (day) => `<tr>
    <td class="cdmx-daycell"><span class="cdmx-day">${day.day}</span><span class="cdmx-daytag">${day.tag || ''}</span></td>
    <td>${T._cdmxStack(day.breakfast)}</td>
    <td>${T._cdmxStack(day.lunch)}</td>
    <td>${T._cdmxStack(day.dinner)}</td>
    <td>${T._cdmxStack(day.activity)}</td>
  </tr>`;
