/* ============================================
   ...
   Single source of truth: data/live.json (loaded by data-loader.js).
   This file pulls the squad + WC history for the current country page
   from window.WC.data once it's ready.
   ============================================ */

document.addEventListener('wc:ready', () => {
  const data = window.WC && window.WC.data;
  if (!data) return;
  const country = location.pathname.split('/').pop().replace('.html', '');

  // Squads → window.playerData (so the existing showPlayer modal keeps working)
  const team = data.teams && data.teams[country];
  if (team && Array.isArray(team.squad)) {
    const map = {};
    team.squad.forEach(p => { map[p.id] = p; });
    window.playerData = map;
  }

  // WC history block (rendered top of country pages)
  const mount = document.getElementById('wc-history');
  const ch = data.countries && data.countries[country];
  if (mount && ch && window.WC.render) {
    mount.innerHTML = window.WC.render.wcHistory(ch);
  }
});
...
// WC history is rendered by the wc:ready handler above.
...
// Decorate one card's weather slot.
async function applyWeatherToOne(card) {
  const slot = card.querySelector('.weather-slot');
  if (!slot || slot.dataset.wxDone) return;
  // Mark immediately so we never enqueue the same card twice
  slot.dataset.wxDone = '1';
  const city = card.dataset.city;
  const date = card.dataset.date;
  const w = await fetchWeather(city, date);
  if (!w) { slot.innerHTML = '<span class="weather-na">— °</span>'; return; }
...
// Concurrency-capped runner — Open-Meteo is happy with ~6 in flight.
async function applyWeatherToCards() {
  const cards = Array.from(document.querySelectorAll('[data-city][data-date]'));
  const pending = cards.filter(c => {
    const s = c.querySelector('.weather-slot');
    return s && !s.dataset.wxDone;
  });
  ...
  async function worker() {
    while (i < pending.length) {
      const idx = i++;
      try { await applyWeatherToOne(pending[idx]); } catch (e) { /* swallow */ }
    }
  }
  await Promise.all(Array.from({ length: Math.min(POOL, pending.length) }, worker));
}

// Run on both events — DOMContentLoaded catches static cards (country pages),
// wc:ready catches cards rendered by the templates after live.json loads
// (schedule.html, anything dynamic). The idempotency flag means cards that
// fired on one event don't refetch on the other.
function _scheduleWeatherSweep() { applyWeatherToCards(); }
document.addEventListener('DOMContentLoaded', _scheduleWeatherSweep);
document.addEventListener('wc:ready',         _scheduleWeatherSweep);
