...
(function () {
  ...
  // Bobby facts come from data/bobby-facts.json (canonical, user-editable)
  let SITE_FACTS = [];
  const SITE_FACTS_LOADED = fetch('data/bobby-facts.json')
    .then(r => r.json())
    .then(j => { SITE_FACTS = (j && j.facts) || []; })
    .catch(() => { SITE_FACTS = []; });
  ...
  function injectFloatingBobbycito() {
    ...
    btn.addEventListener('click', () => {
      const show = () => {
        if (!SITE_FACTS.length) {
          bub.innerHTML = `<span class="bf-tag">¡Órale!</span> Loading Bobby facts…`;
        } else {
          const f = SITE_FACTS[Math.floor(Math.random() * SITE_FACTS.length)];
          bub.innerHTML = `<span class="bf-tag">Bobby fact #${f.number}</span> ${f.text}<a href="bobbycito.html" class="bf-link">Meet Bobbycito →</a>`;
        }
        bub.classList.add('on');
        if (timer) clearTimeout(timer);
        timer = setTimeout(() => bub.classList.remove('on'), 9000);
      };
      // If facts haven't loaded yet, wait for them then show; otherwise show immediately
      if (!SITE_FACTS.length) {
        SITE_FACTS_LOADED.then(show);
      } else {
        show();
      }
    });
